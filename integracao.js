...
export async function clientes(nome, usuario, email, senha, telefone, endereco) {
    try {
        // Exibe no console os dados que estão sendo enviados ao servidor para registro.
        console.log('Enviando dados para registro:', { nome, usuario, email, senha, telefone, endereco });
        ...
        const response = await fetch(`${API_URL}/auth/clientes`, {
            ...
            body: JSON.stringify({ nome, usuario, email, senha, telefone, endereco }) // Converte os dados do registro em uma string JSON e os envia no corpo da requisição.
        });
