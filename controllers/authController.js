...
//       'INSERT INTO dogwalker (name, email, password, birth_date) VALUES (?, ?, ?, ?)',
...
//     await db.promise().query('UPDATE dogwalker SET reset_password_token = ?, reset_password_expires = ? WHERE email = ?', [token, expireDate, email]);
...
//   try {
//     const [user] = await db.promise().query('SELECT * FROM dogwalker WHERE reset_password_token = ? AND reset_password_expires > NOW()', [token]);
...
//     await db.promise().query('UPDATE dogwalker SET password = ?, reset_password_token = NULL, reset_password_expires = NULL WHERE id = ?', [hashedPassword, user[0].id]);
